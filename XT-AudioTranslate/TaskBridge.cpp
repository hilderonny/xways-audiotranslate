...
std::string httpGET(const std::string& server, const std::string& path) {
    ...
    // Adresse auflösen
    if (getaddrinfo(server.c_str(), "80", &hints, &result) != 0) {
        std::cerr << "getaddrinfo failed." << std::endl;
        WSACleanup();
        return "";
    }
    ...
    // Anfrage erstellen (HTTP GET)
    std::string request = "GET " + path + " HTTP/1.1\r\n";
    request += "Host: " + server + "\r\n";
    request += "Connection: close\r\n\r\n"; // "Connection: close" fordert den Server auf, die Verbindung nach der Antwort zu schließen
